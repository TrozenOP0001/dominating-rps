...
function toggleMusic() { musicOn = !musicOn; if (musicOn) startBgMusic(); else stopBgMusic(); $('musicBtn') && ($('musicBtn').innerText = musicOn ? 'Music: On' : 'Music: Off'); save(); }
...
function playerMove(choice) {
  ...
  if (result === 'lose' && state.powerUps.timeFreeze > 0) {
    ...
    cpu = ncpu; result = nr; state.powerUps.timeFreeze = Math.max(0, state.powerUps.timeFreeze - 1); if (attempt > 0) showTemp('Time Freeze used'); 
  }

  if (result === 'lose') {
    if (state.powerUps.shield > 0) { state.powerUps.shield = Math.max(0, state.powerUps.shield - 1); result = 'blocked'; showTemp('Shield blocked your loss'); }
    else if (state.powerUps.extraLife > 0) { state.powerUps.extraLife = Math.max(0, state.powerUps.extraLife - 1); result = 'saved'; showTemp('Extra Life saved you'); }
  }
  ...
  if (result === 'win') {
    let pts = 1;
    if (state.powerUps.triplePoints) pts = 3;
    else if (state.powerUps.doublePoints) pts = 2;
    if (state.powerUps.ultimateStreak) pts = Math.round(pts * (1 + Math.min(4, state.streak * 0.12)));
    if (critArmed) { pts *= 2; state.powerUps.criticalStrikeActive = false; state.powerUps.criticalStrike = Math.max(0, state.powerUps.criticalStrike - 1); showTemp('Critical Strike!'); }
    state.userScore = safeInt(state.userScore + pts);
    state.credits = safeInt(state.credits + 10);
    state.shards = safeInt(state.shards + 1);
    state.streak++;
    state.xp = safeInt(state.xp + 20);
    state.games++;
    spawnParticles(window.innerWidth * 0.5 + (Math.random() - 0.5) * 200, window.innerHeight * 0.5 + (Math.random() - 0.5) * 120, colorFromPalette(0), 22);
    playSfx('win'); grantAchievement('first_win');
  } else if (result === 'lose') {
    state.cpuScore = safeInt(state.cpuScore + 1);
    state.credits = safeInt(state.credits + 3);
    state.streak = 0;
    state.xp = safeInt(state.xp + 8);
    state.games++;
    spawnParticles(window.innerWidth * 0.5, window.innerHeight * 0.5, colorFromPalette(4), 10);
    playSfx('lose');
  } else if (result === 'blocked' || result === 'saved') {
    state.credits = safeInt(state.credits + 6); state.games++; state.xp = safeInt(state.xp + 10); playSfx('click');
  } else { state.credits = safeInt(state.credits + 5); state.games++; state.xp = safeInt(state.xp + 6); playSfx('click'); }

  while (state.xp >= state.xpNext) { state.xp -= state.xpNext; state.level = safeInt(state.level + 1); state.xpNext = Math.round(state.xpNext * 1.25); state.shards = safeInt(state.shards + 5); playSfx('level'); showTemp('Level up!'); }
...
function applyWin(choice, reason) {
  state.userScore = safeInt(state.userScore + 2); state.credits = safeInt(state.credits + 10); state.shards = safeInt(state.shards + 2); state.xp = safeInt(state.xp + 25); state.streak++; state.games++;
  state.lastRound = { user: choice, cpu: 'instant', result: 'win', time: Date.now(), reason };
  spawnParticles(window.innerWidth * 0.5, window.innerHeight * 0.5, colorFromPalette(1), 30);
  playSfx('win'); renderAll();
}

// --- Shops, Achievements, Daily, Leaderboard, Rewind, UI functions ---
// (No changes needed here — they were already syntactically correct. Just ensure all $('id') exist in HTML.)
...
document.addEventListener('DOMContentLoaded', () => {
  // Start tip cycle
  const tipEl = $('tip'); let tipInterval = null;
  if (tipEl) { let i = 0; tipEl.innerText = SPLASH_TIPS[0]; tipInterval = setInterval(() => { i = (i + 1) % SPLASH_TIPS.length; tipEl.innerText = SPLASH_TIPS[i]; }, 1200); }
  ...
  $('startBtn')?.addEventListener('click', startGameFromSplash);
  setTimeout(() => { const splash = $('splash'); if (splash?.style.display !== 'none') startGameFromSplash(); }, 4000);
});}
function stopBgMusic(){ try{ if(bgOsc){ bgOsc.stop(); bgOsc.disconnect(); bgGain.disconnect(); bgOsc=null; bgGain=null; } }catch(e){} }
