/* script.js — Fully fixed for Dominating RPS */

// === STORAGE & STATE ===
const STORAGE_KEY = 'dominating_rps_save_v1';
let musicOn = true;
let state = {
    userScore: 0, cpuScore: 0, games: 0, credits: 0, shards: 0,
    streak: 0, level: 1, xp: 0, xpNext: 100,
    powerUps: {
        doublePoints: 0, triplePoints: 0, shield: 0, extraLife: 0,
        instantWin: 0, predictor: 0, timeFreeze: 0, ultimateStreak: 0,
        criticalStrike: 0, rewind: 0
    },
    achievements: {}, daily: { lastClaim: null, streak: 0 },
    leaderboard: [], lastRound: null, ownedShardShop: {}
};
...
function stopBgMusic(){
    try{ 
        if(bgOsc){ bgOsc.stop(); bgOsc.disconnect(); bgGain.disconnect(); bgOsc=null; bgGain=null; } 
    }catch(e){}
}
...
function playerMove(choice){
    ...
    showTemp(`You chose ${choice}`);
}

// === DUMMY SHOP & ACHIEVEMENTS ===
function renderShops(){ console.log('Render shops'); }
function renderAchievements(){ console.log('Render achievements'); }
function renderDaily(){ console.log('Render daily'); }
function renderLeaderboard(){ console.log('Render leaderboard'); }
function grantAchievement(id){ console.log('Achievement unlocked', id); }
function claimDaily(){ console.log('Daily claimed'); }
...
function showTemp(msg){
    ...
    setTimeout(()=>{ $('resultTitle').innerText='Make a move'; },2400);
}

// === SPLASH / START ===
const SPLASH_TIPS = [
    "💡 Use Predictor wisely.",
    "🔥 Save Prismatic Shards for upgrades.",
    "🎯 Critical Strike doubles points.",
    "🛡️ Shield blocks one loss.",
    "⏳ Time Freeze rerolls CPU if losing."
];
...
document.addEventListener('DOMContentLoaded', ()=>{
    canvas = $('fxCanvas');
    if(canvas){ ctx = canvas.getContext('2d'); resizeCanvas(); fxLoop(); }
    ...
    $('submitScore')?.addEventListener('click', ()=>console.log('Submit score clicked'));
    $('tabCredits')?.addEventListener('click', ()=>console.log('Switch tab credits'));
    $('tabShards')?.addEventListener('click', ()=>console.log('Switch tab shards'));
    $('tabAch')?.addEventListener('click', ()=>console.log('Switch tab achievements'));

    document.addEventListener('keydown', e=>{
        if(e.key==='1') playerMove('rock');
        if(e.key==='2') playerMove('paper');
        if(e.key==='3') playerMove('scissors');
        if(e.key==='m') toggleMusic();
        if(e.key==='r') console.log('Rewind last');
    });
